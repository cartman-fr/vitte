...
pub enum Op {
    PushI64=1, PushStr=2, Add=3, Sub=4, Mul=5, Div=6,
...
fn compile_into(c:&mut Chunk, e:&Expr){
    match e {
        ...
        Expr::Assign{name, e}=>{
            compile_into(c, e); let id=c.add_str(name.clone()); c.emit(Op::StoreG); c.emit_u32(id);
        }
        Expr::Bin{op,a,b}=>{
        ...
        Expr::If{c:a,a:b,b:d}=>{ // rename to avoid collision
            compile_into(c, a);
        ...
        Expr::FnDef{..}|Expr::Lambda{..}|Expr::Tuple(_)|Expr::Record(_)|Expr::Import(_)=>{ /* not implemented */ }
