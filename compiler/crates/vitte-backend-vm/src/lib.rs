...
use vitte_backend_bytecode::{Chunk, Op};
...
impl VM{
    ...
    pub fn run(&mut self, c:&Chunk){
        let code=&c.code;
        let mut rd = |ip:&mut usize|{ let mut b=[0u8;4]; b.copy_from_slice(&code[*ip..*ip+4]); *ip+=4; u32::from_le_bytes(b) };
        while self.ip < code.len(){
            ...
            match op {
                Op::PushI64 => { let id=rd(&mut self.ip); self.stack.push(Val::I64(c.const_i64[id as usize])); }
                Op::PushStr => { let id=rd(&mut self.ip); self.stack.push(Val::Str(c.const_str[id as usize].clone())); }
                ...
                Op::MakeList => { let n=rd(&mut self.ip) as usize; let mut xs=Vec::with_capacity(n); for _ in 0..n { xs.push(self.stack.pop().unwrap()); } xs.reverse(); self.stack.push(Val::List(xs)); }
                ...
                Op::LoadG => { let id=rd(&mut self.ip); let k=c.const_str[id as usize].clone(); let v=self.globals.get(&k).cloned().unwrap_or(Val::Unit); self.stack.push(v); }
                Op::StoreG => { let id=rd(&mut self.ip); let k=c.const_str[id as usize].clone(); let v=self.stack.pop().unwrap_or(Val::Unit); self.globals.insert(k, v); }
                Op::Jmp => { let to = rd(&mut self.ip) as usize; self.ip = to; }
                Op::Jz  => { let to = rd(&mut self.ip) as usize; let v = self.stack.pop().unwrap(); let z = match v { Val::I64(n)=>n==0, Val::Str(s)=>s.is_empty(), Val::List(xs)=>xs.is_empty(), Val::Unit=>true }; if z { self.ip = to; } }
                Op::Halt => break,
            }
        }
    }
